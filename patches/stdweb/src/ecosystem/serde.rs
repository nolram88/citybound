...
use crate::serde_crate::ser::{self, Serialize};
use crate::serde_crate::de::{self, Deserialize, Visitor};
use crate::serde_crate::de::IntoDeserializer;

use crate::webcore::value::{
    self,
    Undefined,
    Null,
    Value
...
use crate::webcore::serialization::{
    JsSerialize,
    SerializedValue
};

use crate::webcore::number::{self, Number, Storage, get_storage};
use crate::webcore::try_from::{TryInto, TryFrom};
use crate::webcore::instance_of::InstanceOf;
use crate::webcore::array::Array;
use crate::webcore::object::Object;
use crate::webcore::global_arena;
...
impl Serialize for Value {
    #[inline]
    fn serialize< S: ser::Serializer >( &self, serializer: S ) -> Result< S::Ok, S::Error > {
        use crate::serde_crate::ser::SerializeMap;
        match *self {
            ...
            Value::Reference( ref reference ) => {
                if Array::instance_of( reference ) {
                    ...
                    let mut map = r#try!( serializer.serialize_map( Some( value.len() ) ) );
                    for (key, value) in value {
                        r#try!( map.serialize_key( &key ) );
                        r#try!( map.serialize_value( &value ) );
                    }

                    map.end()
                } else {
                    let map = r#try!( serializer.serialize_map( None ) );
                    map.end()
                }
            }
        }
